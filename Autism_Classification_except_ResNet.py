    ...
    for folder in os.listdir(test_folders):
        ...
        for j in [random.randrange(0, len(image_ids)) for _ in range(number_of_examples)]:
            ...
            if models:
                ...
                names = ['VGG-16', 'InceptionV3', 'EfficientNet-B0', 'EfficientNet-B7']
...
x = list(range(1, epoches + 1))
model_names = ['VGG-16', 'VGG-16-Dropout', 'InceptionV3', 'EfficientNet-B0', 'EfficientNet-B7']
histories   = [vgg16_history, vgg16_dropout_history, inceptionv3_history,
...
# The 4-model ensemble — VGG-16 (no-dropout), InceptionV3, EfficientNet-B0,
# EfficientNet-B7. VGG-16-Dropout is reported separately as an ablation and
# is NOT part of the ensemble. (ResNet-50 removed.)
...
for folder in os.listdir(test_folders):
    ...
    for image_id in test_image_ids:
        ...
        ensembled_pred_hard.append(mode([v, inc, b0, b7]))

        # Soft voting: average the 4 models' probability vectors, then argmax
        ensemble_probs_all.append(
            np.mean([vgg_probs[0], inc_probs[0], b0_probs[0], b7_probs[0]], axis=0)
        )
...
# ensemble average, since the ensemble is defined over the 4 base
# architectures (one entry per architecture). (ResNet-50 removed.)
EXTRA_FOLD_BUILDERS = {
...
# 7 panels for 7 model_order entries in an 8-slot 2x4 grid — hide the unused slot.
...
models_list = [
    vgg16_final_model, inceptionv3_final_model,
