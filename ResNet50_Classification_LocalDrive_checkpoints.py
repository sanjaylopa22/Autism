...
# safely skip any fold already recorded here.
CV_CHECKPOINT_PATH = MODEL_DIR / 'cv_checkpoint_resnet50.json'

if CV_CHECKPOINT_PATH.exists():
    with open(CV_CHECKPOINT_PATH) as f:
        cv_results = json.load(f)
    completed_folds = set(cv_results['fold'])
...
for fold, (train_idx, val_idx) in enumerate(skf.split(all_image_paths, all_labels)):
    ...
    print(f'ResNet-50 Fold {fold_num} Accuracy: {acc:.4f}')
    ...
    # crashes, this run's progress is not lost.
...
# resnet50_final_model = load_model(
#     str(MODEL_DIR / 'resnet50_model.keras'),
#     custom_objects={'preprocess_input': resnet50_preprocess_input}
# )
