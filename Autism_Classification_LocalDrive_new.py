...
if os.path.exists(_resnet50_path):
    ...
    # FIX: backbone was fully unfrozen (trainable=True), i.e. full fine-tuning
    # of a 25M-parameter network on ~2,300 training images — high overfitting
    # risk, and the likely cause of ResNet-50's test accuracy (63.5%) being
    # far below its 5-fold CV accuracy (82.3%). Freeze it like the other four
    # models so only the new Dense head is trained.
    for layer in resnet50_base.layers:
        layer.trainable = False
    resnet50_x = Flatten()(resnet50_base.output)
    resnet50_x = Dense(256, activation='relu')(resnet50_x)
    resnet50_x = Dense(2, activation='sigmoid')(resnet50_x)
    resnet50_final_model = Model(inputs=resnet50_base.input, outputs=resnet50_x)
...
# The 5-model ensemble (matches the abstract) — VGG-16 (no-dropout), ResNet-50,
# InceptionV3, EfficientNet-B0, EfficientNet-B7. VGG-16-Dropout is reported
# separately as an ablation and is NOT part of the ensemble.
...
def build_fold_resnet50():
    ...
    # FIX: same overfitting-prone full fine-tuning bug as the final model above.
    for layer in base.layers: layer.trainable = False
    x = Flatten()(base.output)
    x = Dense(256, activation='relu')(x)
    x = Dense(2,   activation='sigmoid')(x)
...
# resnet50_final_model       = load_model(str(MODEL_DIR / 'resnet50_model.h5'))
